...
class Scene:
    ...
    def build_balls(self):
        ball_coords = [
                            avango.gua.Vec3(6,2,5),
                            avango.gua.Vec3(-1,0,1),
                            avango.gua.Vec3(6,1,2),
                            avango.gua.Vec3(3,0,4),
                            avango.gua.Vec3(-3,0,4),
                            avango.gua.Vec3(-4,0,2),

                            ]

        up_and_down_animator = UpAndDownAnimator()
        up_and_down_animator.__init__(ball=True)

        first_ball = True
        for ball_coord in ball_coords:
            ball = self.loader.create_geometry_from_file('ball', 'data/objects/sphere.obj', avango.gua.LoaderFlags.MAKE_PICKABLE)
            ball.Tags.value = ['ball',]

            self.apply_material_uniform_recursively(ball, 'Color', avango.gua.Vec4(random.random(), random.random(), random.random() * 2.0, 1.0))

            ball.Transform.value = avango.gua.make_trans_mat(ball_coord.x, ball_coord.y, ball_coord.z)

            if first_ball:
                ball.Transform.connect_from(up_and_down_animator.sf_output_mat)
                first_ball = False

            self.scenegraph['/'].Children.value.append(ball)
...
class UpAndDownAnimator(avango.script.Script):
    ...
    def evaluate(self):
        ...
        if self.is_ball:
            x_pos = 2.0
        self.sf_output_mat.value = avango.gua.make_trans_mat(x_pos, height, 0.0)
