...
class VirtualHandInteraction(avango.script.Script):
    ...
    def apply_gogo(self, threshold):
        ...
        if distance >= 0.3:
           self.hand_node_translate.Transform.value = avango.gua.make_trans_mat(0,0,-20*(distance-0.3)**2)
    ...
    def compute_pick_result(self):
        ...
        direction = (self.hand_node.WorldTransform.value * avango.gua.make_trans_mat(0,0,-1)).get_translate() - self.hand_node.WorldTransform.value.get_translate()
        result = self.picker.compute_all_pick_results(self.hand_node.WorldTransform.value.get_translate(),direction,0.2,[])
        if result:
            return result[0]
        return None
    ...
    def start_dragging(self):
        if self.highlighted_object is not None:
            ...
            position = avango.gua.make_inverse_mat(self.hand_node.WorldTransform.value) * self.dragging_object.Transform.value
            self.hand_node.Children.value.append(self.dragging_object)
            self.dragging_object.Transform.value=position
            self.scenegraph.Root.value.Children.value.remove(self.dragging_object)
