...
class BirdControls(avango.script.Script):
    ...
    def apply_isotonic_rate_control_mapping(self, x_input, y_input):
        ...
        self.velocity.x += x_input * 0.00002
        self.velocity.y += -y_input * 0.00002  
        self.bird_node.Transform.value = self.bird_node.Transform.value * \
    ...
    def apply_isotonic_acceleration_control_mapping(self, x_input, y_input):
        ...
        self.acceleration.x += x_input * 0.00000001
        self.acceleration.y += -y_input * 0.00000001  
    ...
    def apply_elastic_position_control_mapping(self, x_input, y_input):
        ...
        x_offset = x_input * 0.045
        y_offset = -y_input * 0.045 
        self.bird_node.Transform.value = self.bird_start_mat * \
            avango.gua.make_trans_mat(x_offset, y_offset, 0.0)
    ...
    def apply_elastic_rate_control_mapping(self, x_input, y_input):
        ...
        self.velocity.x = x_input * 0.0005
        self.velocity.y = -y_input * 0.0005  
        self.bird_node.Transform.value = self.bird_node.Transform.value * \
            avango.gua.make_trans_mat(self.velocity.x, self.velocity.y, 0.0)
    ...
    def apply_elastic_acceleration_control_mapping(self, x_input, y_input):
        ...
        self.acceleration.x = x_input * 0.000001
        self.acceleration.y = -y_input * 0.000001  
        self.velocity.x += self.acceleration.x
        self.velocity.y += self.acceleration.y
        self.bird_node.Transform.value = self.bird_node.Transform.value * \
            avango.gua.make_trans_mat(self.velocity.x, self.velocity.y, 0.0)
    ...
    def animate_bird(self):
        if self.animation_start_time is not None:
            ...
            speed_constant = 10 #length units/Seconds
            total_diff = (self.animation_target_pos - self.animation_start_pos)
            total_time = total_diff.length() / speed_constant

            elapsed_time = time.time()-self.animation_start_time
            t = (elapsed_time / total_time) #results in the position in the path from 0 to 1
            mod_t = 6 * (t ** 5) - 5 * (t ** 6)
            traveled_diff = avango.gua.Vec3(mod_t * total_diff.x, mod_t * total_diff.y, 0)
            self.bird_node.Transform.value = avango.gua.make_trans_mat(self.animation_start_pos.x, self.animation_start_pos.y, self.animation_start_pos.z) * avango.gua.make_trans_mat(traveled_diff.x, traveled_diff.y, 0.0)
