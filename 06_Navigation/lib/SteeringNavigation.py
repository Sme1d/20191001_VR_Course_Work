...
class SteeringNavigation(avango.script.Script):
    ...
    def evaluate(self):
        ...
        self.sf_navigation_matrix.value *= avango.gua.make_trans_mat(movement_vector)
        self.lf_time = now
    ...
    def apply_position_directed_steering(self, delta_time):
        ...
        forward_vector = self.head_node.WorldTransform.value.get_translate()-self.center_circle.WorldTransform.value.get_translate()
        forward_vector.y=0.0
        distance=forward_vector.length()
        forward_vector.normalize()
        if distance > self.center_circle_diameter / 2:
            speed=distance * 0.5 * self.maximum_speed * delta_time
            movement_vector = forward_vector * speed
            return movement_vector
