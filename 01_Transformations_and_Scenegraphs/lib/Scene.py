...
class Scene:
    ...
    def __init__(self, scenegraph):
        ...
        # ToDo: Find out how to combine multiple rotation matrices
        ...
        transformation_matrices.append(avango.gua.make_trans_mat(0.0, 0.5, 3.0) * avango.gua.make_rot_mat(80,0,1,0) * avango.gua.make_rot_mat(180,0,0,1) * avango.gua.make_rot_mat(-35,1,0,0) * avango.gua.make_scale_mat(1.5,1.5,1.5))
        ...
        own_transformation_matrices.append(self.make_trans_mat(0.0, 0.5, 3.0) * self.make_rot_mat(80,0,1,0) * self.make_rot_mat(180,0,0,1) * self.make_rot_mat(-35,1,0,0) * self.make_scale_mat(1.5,1.5,1.5))
        ...
        own_multiplications.append(self.mult_mat(self.mult_mat(self.mult_mat(self.mult_mat(self.make_trans_mat(0.0, 0.5, 3.0), self.make_rot_mat(80,0,1,0)), self.make_rot_mat(180,0,0,1)), self.make_rot_mat(-35,1,0,0)), self.make_scale_mat(1.5,1.5,1.5)))
        own_multiplications.append(self.mult_mat(self.make_trans_mat(2.0, 1.5, -2.0), self.make_scale_mat(1.5,1,1)))
        own_multiplications.append(self.mult_mat(self.mult_mat(self.mult_mat(self.make_trans_mat(0.0, 3.0, 0.0), self.make_rot_mat(100,0,1,0)), self.make_rot_mat(30,1,0,0)), self.make_rot_mat(5,0,0,1)))
    ...
    def make_rot_mat(self, degrees, ax_x, ax_y, ax_z):
        ...
        radients = math.radians(degrees)
        if ax_x == 1:
            mat.set_element(0,0,1)
            mat.set_element(1,1,math.cos(radients))
            mat.set_element(1,2,-math.sin(radients))
            mat.set_element(2,1,math.sin(radients))
            mat.set_element(2,2,math.cos(radients))
            mat.set_element(3,3,1)
        elif ax_y == 1:
            mat.set_element(1,1,1)
            mat.set_element(0,0,math.cos(radients))
            mat.set_element(0,2,math.sin(radients))
            mat.set_element(2,0,-math.sin(radients))
            mat.set_element(2,2,math.cos(radients))
            mat.set_element(3,3,1)
        else:
            mat.set_element(2,2,1)
            mat.set_element(0,0,math.cos(radients))
            mat.set_element(0,1,-math.sin(radients))
            mat.set_element(1,0,math.sin(radients))
            mat.set_element(1,1,math.cos(radients))
            mat.set_element(3,3,1)
    ...
    def mult_mat(self, lhs, rhs):
        ...
        for x in range(4):
            for y in range(4):
                val = lhs.get_element(x,0)*rhs.get_element(0,y)+lhs.get_element(x,1)*rhs.get_element(1,y)+lhs.get_element(x,2)*rhs.get_element(2,y)+lhs.get_element(x,3)*rhs.get_element(3,y)
                result.set_element(x, y, val)
    ...
    def build_rotating_monkeys(self):
        ...
        node1 = avango.gua.nodes.TransformNode(Name = "translate")
        node2 = avango.gua.nodes.TransformNode(Name = "rotate")

        node1.Transform.value = avango.gua.make_trans_mat(0.0, 3.0, -12.0)
        node2.Transform.value = avango.gua.make_rot_mat(45, 0, 1, 0)
        self.big_monkey.Transform.value = avango.gua.make_scale_mat(2.5)

        self.scenegraph.Root.value.Children.value.append(node1)
        node1.Children.value.append(node2)
        node2.Children.value.append(self.big_monkey)
        # YOUR CODE - END (Exercise 1.5 - Node Structure for big_monkey)

        # YOUR CODE - BEGIN (Exercise 1.6 - Field Connection)
        animator = RotationAnimator()
        self.big_monkey.Transform.connect_from(animator.sf_rot_mat)
        ...
        self.another_big_monkey.Material.value.set_uniform('Color', avango.gua.Vec4(1.0, 0.95, 0.55, 1.0))
        ...
        node3 = avango.gua.nodes.TransformNode(Name = "static rotate")
        node4 = avango.gua.nodes.TransformNode(Name = "translate")

        node4.Transform.value = avango.gua.make_trans_mat(8.0, 1.5, 0.0)
        self.another_big_monkey.Transform.value = avango.gua.make_scale_mat(2.5)

        self.scenegraph.Root.value.Children.value.append(node3)
        node3.Children.value.append(node4)
        node4.Children.value.append(self.another_big_monkey)

        node3.Transform.connect_from(animator.sf_rot_mat)
...
class WorldTransformComputer(avango.script.Script):
    ...
    def compute_world_transform(self, node):
        ...
        path.append(node.Transform.value)
        while parent:
            path.append(parent.Transform.value)
            parent = parent.Parent.value

        result_mat = avango.gua.make_identity_mat()
        result_mat = path.pop()
    ...
    def evaluate(self):
        ...
        check = self.compute_world_transform(self.sf_node.value)
        print("World T:\n", check)
        print("Translation: ",check.get_translate())
        print("Rotation:    ",check.get_rotate())
        print("Scale:       ",check.get_scale())
