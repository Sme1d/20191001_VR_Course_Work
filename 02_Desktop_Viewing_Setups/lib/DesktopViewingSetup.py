...
class DesktopViewingSetup(avango.script.Script):
    ...
    def create(self, scenegraph):
        ...
        self.around_bird_rotation = avango.gua.nodes.TransformNode(Name='around_bird_rotation')
        follow_bird_translation = avango.gua.nodes.TransformNode(Name='follow_bird_translation')
        follow_bird_translation.Transform.value = avango.gua.make_trans_mat(.0, 0.0, 5.0)
        self.scenegraph['/bird_rot_animation/bird_transform'].Children.value.append(self.around_bird_rotation)
        self.around_bird_rotation.Children.value.append(follow_bird_translation)

        self.camera_rotation = avango.gua.nodes.TransformNode(Name='camera_rotation')
        follow_bird_translation.Children.value.append(self.camera_rotation)
        ...
        self.screen_node.Transform.value = avango.gua.make_trans_mat(0.0, 0.0, -0.6)

        # YOUR CODE - BEGIN (Exercises 2.3, 2.5, 2.6, 2.7 - Attach Screen Node)
        #self.scenegraph['/static_transform'].Children.value.append(self.scene_node)
        #self.scenegraph['/bird_rot_animation/bird_transform'].Children.value.append(self.screen_node)
        #follow_bird_translation.Children.value.append(self.screen_node)
        self.camera_rotation.Children.value.append(self.screen_node)
        ...
        self.camera_node.BlackList.value.append("invisible")
        ...
        #follow_bird_translation.Children.value.append(self.camera_node)
        self.camera_rotation.Children.value.append(self.camera_node)
        ...
        self.keyboard_sensor = avango.daemon.nodes.DeviceSensor(DeviceService=self.device_service)
    ...
    def evaluate(self):
        ...
        new_angle = rotation_speed * elapsed_time

        # YOUR CODE - BEGIN (Exercise 2.6 - Map Left and Right Arrow Keys)
        if self.sf_left_arrow_key.value:
            self.camera_rotation.Transform.value*=avango.gua.make_rot_mat(new_angle,0,1,0)
        ...
        if self.sf_up_arrow_key.value:
            self.around_bird_rotation.Transform.value*=avango.gua.make_rot_mat(new_angle,1,0,0)
    ...
    def sf_visibility_toggle_changed(self):
        if self.sf_visibility_toggle.value:
            ...
            if "invisible" in node_to_toggle.Tags.value:
                node_to_toggle.Tags.value = []
            else:
                node_to_toggle.Tags.value = ["invisible"]
    ...
    def compute_fov_in_deg(self):
        ...
        #vector = self.camera_node.Transform.value.get_translate() - self.screen_node.Transform.value.get_translate()
        #magnitude = math.sqrt(math.pow(vector[0], 2) + math.pow(vector[1], 2) + math.pow(vector[2], 2))
        #tangent = SCREEN_SIZE[0] / 2.0 / magnitude
        distance = self.camera_node.Transform.value.get_element(2,3) - self.screen_node.Transform.value.get_element(2,3)
        tangent = SCREEN_SIZE[0] / 2.0 / distance
        angle = math.degrees(math.atan(tangent))
        return 2.0 * angle
        # YOUR CODE - END (Exercise 2.1 - Compute FOV from Camera-Screen Relation)
    ...
    def set_fov_in_deg(self, degrees):
        ...
        distance = (SCREEN_SIZE[0] / 2) / math.tan(math.radians(degrees/2))
        self.screen_node.Transform.value = avango.gua.make_trans_mat(0.0, 0.0, -distance)
    ...
    def compute_model_view_transform(self, node):
        ...
        ModelTransform = node.WorldTransform.value
        ViewTransfrom = avango.gua.make_inverse_mat(self.screen_node.WorldTransform.value)
        ModelViewTransform = ViewTransfrom * ModelTransform
